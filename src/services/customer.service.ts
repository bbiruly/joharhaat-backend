...
export const listWishlist = (userId: string) => prisma.wishlistItem.findMany({ where: { userId }, include: { product: { include: { variants: { where: { isActive: true } }, vendor: { select: { businessName: true } }, category: true, media: { orderBy: [{ isCover: 'desc' }, { sortOrder: 'asc' }], select: { id: true, url: true, altText: true, isCover: true, sortOrder: true, width: true, height: true } } } } }, orderBy: { createdAt: 'desc' } });
